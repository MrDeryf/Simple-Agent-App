...
def get_nodes(llm:BaseChatModel, evaluator:BaseChatModel) -> list[Callable[[Mapping[str, object]], Mapping[str, object]]]:
    ...
    def llm_call_generator(state: Mapping[str, object]) -> Mapping[str, object]:
        ...
        if state.get("feedback"):
            messages.append(
                HumanMessage(content=f"Write a joke about {state['topic']} but take into account the feedback: {state['feedback']}"))
